...
import "./NebuConfig.sol";

contract Nebutroller is Comptroller {
    /// @notice Emitted when an admin delists a market
    event MarketDelisted(CToken cToken);

    NebuConfig public nebuConfig;
   ...
   function _delistMarket(CToken cToken) external {
    ...
    function _setQsConfig(NebuConfig _nebuConfig) public {
        require(msg.sender == admin);

        nebuConfig = _nebuConfig;
    }
    ...
    function calculateSeizeTokenAllocation(uint _seizeTokenAmount) public view returns(uint liquidatorAmount, uint safetyVaultAmount) {
        return nebuConfig.calculateSeizeTokenAllocation(_seizeTokenAmount, liquidationIncentiveMantissa);
    }
    
    function transferComp(address user, uint userAccrued, uint threshold) internal override returns (uint) {
        ...
        if (userAccrued >= threshold && userAccrued > 0 && compAddress != address(0x0)) {
            ...
            if (userAccrued <= compRemaining) {
                (uint userAmount, uint governanceAmount) = nebuConfig.getCompAllocation(user, userAccrued);
                if (userAmount > 0) comp.transfer(user, userAmount);
                if (governanceAmount > 0) comp.transfer(nebuConfig.safetyVault(), governanceAmount);
                return 0;
            }
        }
        return userAccrued;
    ...
    function borrowAllowed(address cToken, address borrower, uint borrowAmount) external override returns (uint) {
        ...
        if (!markets[cToken].accountMembership[borrower]) {
            ...
            Error err0 = addToMarketInternal(CToken(msg.sender), borrower);
        ...
        uint borrowCap = nebuConfig.getBorrowCap(cToken);
    ...
    function flashLoanAllowed(address cToken, address to, uint256 flashLoanAmount) view public returns (uint) {
        // Pausing is a very serious situation - we revert to sound the alarms
        require(!borrowGuardianPaused[cToken], "p");
        require(nebuConfig.whitelist(to), "!w");

        if (!markets[cToken].isListed) {
            return uint(Error.MARKET_NOT_LISTED);
        }

        uint flashLoanCap = nebuConfig.getFlashLoanCap(cToken);
    ...
    function getFlashLoanCap(address cToken) view external returns (uint) {
        return nebuConfig.getFlashLoanCap(cToken);
    }
    ...
    function mintAllowed(address cToken, address minter, uint mintAmount) external override returns (uint) {
        require(!nebuConfig.isBlocked(minter));
        // Pausing is a very serious situation - we revert to sound the alarms
        ...
        uint supplyCap = nebuConfig.getSupplyCap(cToken);
  ...
  */
    function updateCompSupplyIndex(address cToken) internal override {
     ...
     */
    function updateCompBorrowIndex(address cToken, Exp memory marketBorrowIndex) internal override {
 ...
 */
    function getHypotheticalAccountLiquidityInternal(
        ...
        CToken cTokenModify,
        uint redeemTokens,
        uint borrowAmount) internal view override returns (Error, uint, uint) {

        // If credit limit is set to MAX, no need to check account liquidity.
        if (nebuConfig.getCreditLimit(account) == type(uint).max) {
            return (Error.NO_ERROR, type(uint).max, 0);
        }
        ...
        if (nebuConfig.getCreditLimit(account) > 0) {
            vars.sumCollateral = nebuConfig.getCreditLimit(account);
        }
    ...
    function liquidateBorrowAllowed(
        ...
        address liquidator,
        address borrower,
        uint repayAmount) public override returns (uint) {
        require(nebuConfig.getCreditLimit(borrower) == 0 , "c");
    ...
    function repayBorrowAllowed(
        ...
        address borrower,
        uint repayAmount) public override returns (uint) {
        require(nebuConfig.getCreditLimit(borrower) == 0 || payer == borrower);

        return super.repayBorrowAllowed(cToken, payer, borrower, repayAmount);
    }

    function _supportMarket(CToken cToken) external override returns (uint) {
        if (msg.sender != admin) {
        ...
        newMarket.borrowFactorMantissa = 1e18;
    ...
    function redeemVerify(address, address redeemer, uint, uint) external override view {
       require(!nebuConfig.isBlocked(redeemer), "b");
    }
    ...
    /**
      ...
      */
    function _setPriceOracle(PriceOracle newOracle) external override returns (uint) {
        // Check caller is admin
        require(msg.sender == admin);
    ...
    function safetyGuardian() external view override returns (address) {
        return nebuConfig.safetyGuardian();
    }
