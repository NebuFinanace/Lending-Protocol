 ...
 * @author Compound
 */
abstract contract CWrappedNative is NToken, CErc20Storage, CCapableDelegateInterface {
    /**
    ...
    function doFlashLoanTransferOut(address receiver, address wrapperToken, uint amount) internal override virtual {
    ...
    function doFlashLoanTransferIn(address receiver, address wrapperToken, uint amount) internal override virtual {
        require(wrapperToken == underlying, "!token");
        IWETH nativeWrapper = IWETH(underlying);
        uint balanceBefore = nativeWrapper.balanceOf(address(this));
    ...
    function isNativeToken() public pure override virtual returns (bool) {
        return true;
    }

    function validateFlashloanToken(address token) view internal override virtual {
