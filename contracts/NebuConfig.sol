...
contract NebuConfig is Ownable, Exponential {
    ...
    constructor(NebuConfig previousNebuConfig) {
        safetyGuardian = msg.sender;
        if (address(previousNebuConfig) == address(0x0)) return;
       
        safetyGuardian = previousNebuConfig.safetyGuardian();
        compToken = previousNebuConfig.compToken();
        safetyVaultRatio = previousNebuConfig.safetyVaultRatio();
        safetyVault = previousNebuConfig.safetyVault();
    }
